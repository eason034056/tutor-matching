...
test('sendTutorCaseNotificationEmail 逐封發送給多位教師', async () => {
  const sentMessages = []
  ...
  try {
    ...
    // 驗證逐封發送（每位教師一封）
    assert.equal(sentMessages.length, 3, '預期發送 3 封獨立郵件')
    assert.equal(sentMessages[0].to, 'tutor1@example.com')
    assert.equal(sentMessages[1].to, 'tutor2@example.com')
    assert.equal(sentMessages[2].to, 'tutor3@example.com')
...
test('sendTutorCaseNotificationEmail 部分失敗不影響其他封', async () => {
  let callCount = 0
  const originalEnv = { ...process.env }
  ...
  try {
    const { sendTutorCaseNotificationEmail } = loadModule({
      onSendMail: (options) => {
        callCount++
        // 第 2 封模擬失敗
        if (options.to === 'fail@example.com') throw new Error('SMTP error')
      },
    })

    const result = await sendTutorCaseNotificationEmail({
      recipientEmails: ['ok1@example.com', 'fail@example.com', 'ok2@example.com'],
      notification: {
        caseNumber: 'CS-PARTIAL',
        subject: '測試',
    ...
    assert.equal(callCount, 3, '三封都應嘗試發送')
    assert.equal(result.successCount, 2)
    assert.equal(result.recipientCount, 3)
    assert.deepEqual(result.failedEmails, ['fail@example.com'])
  } finally {
    Object.keys(process.env).forEach((key) => {
      if (!(key in originalEnv)) delete process.env[key]
    })
...
test('notifyApprovedTutors 完整流程：撈教師 → 組資料 → 逐封發郵件', async () => {
  const sentMessages = []
  const originalEnv = { ...process.env }
  ...
  try {
    ...
    assert.equal(sentMessages[1].to, 'bob@example.com')
